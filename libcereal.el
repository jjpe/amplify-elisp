...
(cl-defun cereal/msg (process request-number kind
  ...
  (let* ((msg (cereal/msg-new)))
    ...
    (cond ((stringp contents)
           (cereal/msg-set-contents msg (cereal/contents-new-text contents)))
          ((listp contents)
           (cereal/msg-set-contents msg (apply #'cereal/contents-new-entries contents)))
          ((user-ptrp contents)
    ...
    (dolist (region regions)
      (cond ((user-ptrp region)
            ...
            ((listp region)
             (->> (cereal/region-new (plist-get region :begin)
                                     (plist-get region :end))
                  (cereal/msg-add-region msg)))
            (t (error "[cereal/msg] Invalid regions value: %s" regions))))
    (cond ((stringp contents)
           (cereal/msg-set-language msg (cereal/language-new language)))
          ((user-ptrp language)
